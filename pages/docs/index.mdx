...
### Connector - run Fabric mods on NeoForge! [#connector]

Connector is a translation/compatibility layer that allows running [Fabric](https://fabricmc.net/) mods on [NeoForge](https://neoforged.net/).
...
Visit Connector's [website](https://connector.sinytra.org) for more information or
read the [documentation](https://moddedmc.wiki/en/project/connector/latest/docs) for mod developers and modpack authors.
...
The Forgified Fabric API (FFAPI) is a **direct port** of Fabric API to NeoForge, regularly kept up to date with the upstream repository.
...
<Callout type="info">
    Starting 1.20.4, NeoForge comes with the Fabric fork of Mixin by default, making the use of Mixin Transmogrifier no
    longer necessary.
</Callout>
